...
class CustomListUserAdapter (val context: Context,
                             val modelUser: List<ModelUser>) : BaseAdapter(){
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        ...
        //set value
        var urlAvatar = modelUser[position].avatar
        Glide.with(context).load(urlAvatar).into(holder.avatar!!)
//        Picasso.get().load(urlAvatar).into(holder.avatar)

        var value_fullname = modelUser[position].first_name+" "+modelUser[position].last_name
        holder.fullname!!.text = value_fullname

        var value_email = modelUser[position].email
